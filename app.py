...
lineaccesstoken = '00kz0onoPrX9a6b3Qiz9lNHwxyaVmYB+ExMffjoxtBeDcC9qKCV/0JI6qW5i8qMd0IJ58l4TClHT1rsM6P1wtcU/8BZ/uisUMCEZWWgigrrjJknMEsg8a1AY8Estorzcl5mUECrBeDQGIyjNXn5G1QdB04t89/1O/w1cDnyilFU='
...
def event_handle(event,json_line):
    ...
    if msgType == "text":
        ...
        if msg == "สวัสดี":
            replyObj = TextSendMessage(text="สวัสดีค่ะ นายท่าน")
            line_bot_api.reply_message(rtoken, replyObj)
        elif msg == "หิวไหม":
            replyObj = TextSendMessage(text="นิดหน่อยอ่ะ")
            line_bot_api.reply_message(rtoken, replyObj)
        elif msg == "ไปทะเลกัน":
            replyObj = TextSendMessage(text="ฉันอยากอยู่คนเดียว บ้ายบาย อิอิ")
            line_bot_api.reply_message(rtoken, replyObj)
        else :
            headers = request.headers
            json_headers = ({k:v for k, v in headers.items()})
            json_headers.update({'Host':'bots.dialogflow.com'})
            url = "https://dialogflow.cloud.google.com/v1/integrations/line/webhook/02285c2f-5640-40d2-9eb3-c9a24eda2cf8"
